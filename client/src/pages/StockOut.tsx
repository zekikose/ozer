...
const StockOut: React.FC = () => {
  ...
  const stockOutMutation = useMutation(
    async (data: StockOutForm) => {
      ...
      const response = await axios.post('/api/stock/out', requestData);
    ...
    },
    {
      ...
      onError: (error: any) => {
        console.error('Stock out error:', error.response?.data);
        const errorMessage = error.response?.data?.error || 'Stok çıkışı başarısız';
  ...
  const onSubmit = async (data: StockOutForm) => {
    ...
    try {
      ...
      await stockOutMutation.mutateAsync(formData);
      setItems([]);
    } finally {
  ...
  const updateItem = (index: number, field: keyof StockOutItem, value: string | number) => {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  type="datetime-local"
                  {...register('exit_date', { required: 'Çıkış tarihi gereklidir' })}
                  defaultValue={new Date().toISOString().slice(0, 16)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                {errors.exit_date && (
                  <p className="mt-1 text-sm text-red-600">{errors.exit_date.message}</p>
                )}
              </div>
            </div>
